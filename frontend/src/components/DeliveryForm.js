...
function DeliveryForm({ user, onDeliveryCreated }) {
  const [formData, setFormData] = useState({
    customer: "",
    warehouse: user.warehouse,
    items: [{ product: "", quantityOrdered: 0 }],
    notes: "",
  })
  ...
  const addItem = () => {
    setFormData({
      ...formData,
      items: [...formData.items, { product: "", quantityOrdered: 0 }],
    })
  }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      await axios.post(`${API_URL}/operations/deliveries`, formData, { withCredentials: true })
      onDeliveryCreated()
      setError("")
    } catch (err) {
      setError(err.response?.data?.error || "Error creating delivery")
    }
  }
          ...
          <div key={index} className="grid grid-cols-3 gap-4 mb-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleItemChange(index, "quantityOrdered", Number.parseInt(e.target.value))}
                required
              />
            </div>
